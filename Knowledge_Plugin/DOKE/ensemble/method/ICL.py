...
def gen_data(user, history, candidate, target, item2title, dataset):
    ...
    prompt += "["+", ".join([f"{idx}. {item2title[x]}" for idx, x in enumerate(history[:-1])])+"]\n"
    prompt += f"Then if I ask you to recommend a new {words['movie']} to me according to the given {words['watching']} history, you should recommend {item2title[history[-1]]}.\n"
    prompt += f"And now that I've just {words['watched']} {item2title[history[-1]]}\n"
    ...
    # prompt += f"In order for you to make better recommendations, I will give you some {words['movies']} that I may be interested in in the candidate list according to my history, and ranked in descending order. They are:\n"

    # U2I_data = json.loads(open(f"../data/{dataset}/SASRec_U2I_candidate_pop.json").readline())
    # prompt += ", ".join([item2title[x[0]] for x in U2I_data[user][:20]])+".\n"
